...
export default function DedupSchools({ session }) {
  ...
  const mergeSchools = async (keepSchool, deleteSchool) => {
    ...
    try {
      ...
      // Active-status merge: if either record is active, the surviving
      // record should be active. Mirrors the coach dedup smart-merge.
      // Default-undefined is treated as active for back-compat.
      const keepActive = keepSchool.is_active !== false
      const deleteActive = deleteSchool.is_active !== false
      if (!keepActive && deleteActive) {
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  } ${school1.is_active === false ? 'bg-orange-50' : ''}`}>
                    <div className="font-medium text-lg">
                      {school1.school}
                      {school1.is_active === false && (
                        <span className="ml-2 inline-block text-xs font-normal bg-orange-200 text-orange-900 px-2 py-0.5 rounded-full align-middle">
                          Inactive
                        </span>
                      )}
                    </div>
                  ...
                  <div className={`p-3 rounded-lg border-2 ${
                    isSelected ? 'border-blue-300' : 'border-gray-200'
                  } ${school2.is_active === false ? 'bg-orange-50' : ''}`}>
                    <div className="font-medium text-lg">
                      {school2.school}
