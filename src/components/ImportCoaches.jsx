...
export default function ImportCoaches({ session }) {
  ...
  // Keeps inactive schools in the result set so bulk-import matching
  // can surface them — useful when re-importing source data after an
  // admin has marked a school inactive. The preview UI badges
  // inactive matches so the admin can decide whether to reactivate.
  useEffect(() => {
    async function loadSchools() {
      ...
      while (true) {
        const { data, error } = await supabase
          .from('schools')
          .select('id, school, city, state, division, is_active')
          .order('school')
                        ...
                        importMode === 'update' ? (
                          <tr key={row.id} className={!row.include ? 'bg-gray-50 opacity-60' : ''}>
                            ...
                            <td className="px-3 py-2 text-sm text-gray-600">
                              ...
                              {row.matchedSchool?.is_active === false && (
                                <span className="ml-2 inline-block text-xs bg-orange-100 text-orange-800 px-2 py-0.5 rounded-full whitespace-nowrap">
                                  Inactive
                                </span>
                              )}
                            </td>
                              ...
                              >
                                ...
                                {schools.map(s => (
                                  <option key={s.id} value={String(s.id)}>
                                    {s.school}{s.is_active === false ? ' (Inactive)' : ''}
                                  </option>
                                ))}
                              </select>
                              {row.matchedSchool?.is_active === false && (
                                <span className="ml-2 inline-block text-xs bg-orange-100 text-orange-800 px-2 py-0.5 rounded-full whitespace-nowrap align-middle">
                                  Inactive
                                </span>
                              )}
                            </td>
