 ...
 * metrics read), and the active coaching staff — once — then derives the basics
 * the cards share. Metrics compute from whatever seasons exist (coverage-agnostic).
 *
...
export function useProgramProfile(client, schoolId) {
  const [state, setState] = useState({
    loading: true, error: null, school: null, rosters: [], coaches: [],
  })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const [schoolRes, rostersRes, coachesRes] = await Promise.all([
        ...
        if (!cancelled) {
          setState({ loading: false, error: e?.message || String(e), school: null, rosters: [], coaches: [] })
        }
      }
    })()
  ...
  return { ...state, seasons, currentSeason, currentRoster, lastSyncedRaw }
}
