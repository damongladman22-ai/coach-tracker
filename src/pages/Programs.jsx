...
export default function Programs({ session }) {
  ...
  const handleDelete = async (item) => {
    if (
      !confirm(
        `Delete program "${item.name}"? Will fail if any teams reference it. Mark Inactive instead to retire it.`
      )
    )
      return
  ...
  return (
    <AdminLayout session={session} title="Programs">
      <div className="mb-6">
        <button
          onClick={() => {
            resetForm()
